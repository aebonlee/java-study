...
## 학습 커리큘럼 (5개 과정, 53개 레슨 + 8개 프로젝트 = 61개 콘텐츠)
...
### Course 4: 실무 Java (10개 레슨)
| ID | 제목 | 주요 내용 |
|----|------|----------|
| PR01 | IntelliJ IDEA 완벽 가이드 | 설치, 디버깅, 단축키, 플러그인 |
| PR02 | Git과 GitHub 버전 관리 | git 명령어, 브랜치, merge/rebase, PR |
| PR03 | Maven과 Gradle 빌드 도구 | pom.xml, build.gradle, 의존성, 멀티모듈 |
| PR04 | SQL 기초 | DDL/DML, JOIN, 서브쿼리, 인덱스 |
| PR05 | JDBC 심화와 트랜잭션 | ACID, 격리수준, HikariCP, DAO |
| PR06 | JSON 처리 (Jackson, Gson) | ObjectMapper, DTO 변환, REST 연동 |
| PR07 | 클린 코드와 리팩토링 | SOLID, 네이밍, 코드 스멜, 리팩토링 |
| PR08 | 로깅과 디버깅 | SLF4J, Logback, MDC, 디버깅 전략 |
| PR09 | Java 날짜/시간 API | LocalDate/Time, Duration, Formatter |
| PR10 | Linux와 서버 환경 | Linux 명령어, WAS, JAR/WAR, 클라우드 |

### 프로젝트 (4레벨, 8프로젝트)
| 레벨 | ID | 제목 | 난이도 | 선수 과목 |
|------|-----|------|--------|-----------|
| 기초 | PJ01 | 콘솔 주소록 | beginner | Ch01-06, Ch08 |
| 기초 | PJ02 | 학생 성적 관리 시스템 | beginner | Ch01-08 |
| 심화 | PJ03 | 도서 관리 시스템 | intermediate | Ch01-12, PR01/03/04/05 |
| 심화 | PJ04 | 멀티스레드 채팅 | intermediate | Ch01-12, PR01/07/08 |
| 웹 | PJ05 | 서블릿 게시판 | advanced | S01-S10, PR04/05/06 |
| 웹 | PJ06 | 서블릿 쇼핑몰 | advanced | S01-S10, PR04/05/06/07 |
| Spring | PJ07 | Spring REST 블로그 API | expert | SP01-SP15, PR02/03/04 |
| Spring | PJ08 | Spring E-Commerce 플랫폼 | expert | SP01-SP16, PR02/03/04/10 |

## 기능 목록
- [x] 53개 학습 레슨 (Java 17 + 서블릿 10 + 스프링 16 + 실무 10) + 8개 프로젝트
- [x] 학습 진도 추적 (localStorage)
- [x] 다크 모드
- [x] 반응형 디자인
- [x] OG 메타 태그 (카카오/SNS 공유)
- [x] Supabase 연동 준비
- [x] 퀴즈 센터 (7단계 × 10문제, 타이머, 셔플)
- [x] 도장깨기 (44개 배지, 4등급)
- [x] 마이페이지 (통계, 수료증 7종, 성적표, 16단계 진도)
- [x] 이용 가이드 (11개 가이드, FAQ)
- [x] Navbar 진도 원형 그래프
- [x] 배지 획득 실시간 알림 팝업
- [x] 로그인 시스템 (Google/Kakao OAuth + 이메일)
- [x] 세션 관리 (30분 자동 만료, 경고 배너)
- [x] Supabase 진도 동기화 (로그인 시 합집합 병합)
- [x] Supabase 배지 동기화
- [x] 이메일 로그인/회원가입 (v3.4.0)
...
- [x] 메가 드롭다운 네비게이션 (자바학습하기 4섹션 + 스프링 2섹션 + 실무 4섹션 + 프로젝트 4섹션, 2x2 그리드)
- [x] 퀴즈 확장 (4개 → 7개 퀴즈, 서블릿/스프링/실무 추가)
- [x] 배지 확장 (24개 → 44개, 서블릿/스프링/실무/프로젝트 대응)
- [x] 수료증 확장 (4종 → 7종, 서블릿/Spring/실무 수료증 추가)
- [x] 실무 Java 과정 (10개 레슨, 전용 허브 페이지, v3.5.0)
- [x] 페이지 헤더 디자인 통일 (다크 그라데이션, 5개 페이지)
- [x] 학습완료 버튼 컬러 통일 (btn-primary, 43개 레슨)
- [x] 프로젝트 섹션 (8개 프로젝트, 4레벨, 허브 페이지, v3.6.0)
- [x] 프로젝트 배지 5개 추가 (총 44개)
- [x] 메가 드롭다운 2x2 레이아웃 (자바학습하기/실무/프로젝트)
- [x] 컬러 통일 (메가 드롭다운 섹션 헤더, 퀴즈 카드)
- [x] 코드 실행기 복사 버튼 (JavaCodeRunner 제목 옆 복사 버튼)
- [x] 전체 커리큘럼 통합 (JavaLearning 5개 과정 61개 콘텐츠 표시)
