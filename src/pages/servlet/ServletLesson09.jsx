...
export default function ServletLesson09() {
    ...
    <main className="java-lesson">
      <div className="page-header">
        <div className="container">
          ...
          <h1>Lesson 09. 파일 업로드와 DB 연동</h1>
          <p>서블릿에서 파일 업로드를 처리하고, JDBC를 활용하여 데이터베이스와 연동하는 방법을 학습합니다.</p>
        </div>
      </div>
      <div className="container">
        <div className="lesson-content">
          ...
          <h2>1. 파일 업로드 개요</h2>
          <p>
            파일 업로드는 <code>multipart/form-data</code> 인코딩 타입으로 전송됩니다.
            서블릿 3.0부터 별도의 라이브러리 없이 파일 업로드를 처리할 수 있습니다.
          </p>
          <div className="code-block">
            <div className="code-header"><span className="file-name"><i className="fas fa-file-code"></i> upload.html</span></div>
...
<body>
    ...
    <form action="upload" method="POST" enctype="multipart/form-data">
        ...
        <button type="submit">업로드</button>
          ...
          {/* 2. @MultipartConfig */}
          <h2>2. @MultipartConfig 설정</h2>
          <p>파일 업로드를 처리하는 서블릿에는 <code>@MultipartConfig</code> 어노테이션을 추가해야 합니다.</p>
          <div className="code-block">
            <div className="code-header"><span className="file-name"><i className="fas fa-file-code"></i> UploadServlet.java</span></div>
...
public class UploadServlet extends HttpServlet {
    // ...
          ...
          {/* 3. Part 인터페이스 */}
          <h2>3. Part 인터페이스로 파일 처리</h2>
          <div className="code-block">
            <div className="code-header"><span className="file-name"><i className="fas fa-file-code"></i> UploadServlet.java (전체)</span></div>
            <pre><code>{`@WebServlet("/upload")
    ...
    private String extractFileName(Part part) {
        ...
        return null;
          ...
          <div className="callout callout-tip">
            ...
            <p>실무에서는 파일을 웹 서버가 아닌 별도의 파일 서버(AWS S3 등)에 저장하고, DB에는 파일 경로만 저장합니다. UUID를 사용하면 파일명 충돌을 방지할 수 있습니다.</p>
          </div>
          ...
          <p>
            ...
            주요 인터페이스는 <code>Connection</code>, <code>Statement</code>, <code>ResultSet</code>입니다.
          </p>
          <div className="code-block">
            <div className="code-header"><span className="file-name"><i className="fas fa-file-code"></i> JDBCBasic.java</span></div>
...
public class JDBCBasic {
    public static void main(String[] args) {
        ...
        try {
          ...
          {/* 5. JDBC CRUD */}
          <h2>5. JDBC로 CRUD 구현</h2>
          <div className="code-block">
            <div className="code-header"><span className="file-name"><i className="fas fa-file-code"></i> MemberDAO.java (CRUD)</span></div>
            <pre><code>{`public class MemberDAO {
    ...
    public int delete(int id) throws SQLException {
        ...
        try (Connection conn = getConnection();
          ...
          {/* 6. Connection Pool */}
          <h2>6. Connection Pool (커넥션 풀)</h2>
          <p>
            매 요청마다 DB 연결을 생성/해제하면 성능이 크게 저하됩니다.
            커넥션 풀은 미리 일정 수의 연결을 만들어두고 재사용합니다.
          </p>
          <div className="code-block">
            <div className="code-header"><span className="file-name"><i className="fas fa-file-code"></i> context.xml (Tomcat DBCP 설정)</span></div>
            <pre><code>{`<!-- META-INF/context.xml -->
<Context>
    <Resource name="jdbc/mydb"
          ...
          <div className="code-block">
            <div className="code-header"><span className="file-name"><i className="fas fa-file-code"></i> DBUtil.java (커넥션 풀 사용)</span></div>
...
public class DBUtil {
    ...
    // Connection.close() 호출 시 풀로 반환됨
          ...
          <div className="callout callout-info">
            <div className="callout-title"><i className="fas fa-info-circle"></i> HikariCP</div>
            <p>Spring Boot에서는 기본 커넥션 풀로 HikariCP를 사용합니다. 가장 빠른 JDBC 커넥션 풀로, 설정이 간단하고 성능이 뛰어납니다.</p>
          </div>

          {/* 7. SQL Injection 방지 */}
          <h2>7. PreparedStatement로 SQL Injection 방지</h2>
          <p>
            <code>Statement</code>는 SQL Injection 공격에 취약합니다.
            반드시 <code>PreparedStatement</code>를 사용하여 파라미터 바인딩 방식으로 쿼리를 실행해야 합니다.
          </p>
          <div className="code-block">
            <div className="code-header"><span className="file-name"><i className="fas fa-file-code"></i> SQL Injection 비교</span></div>
            <pre><code>{`// ❌ 위험! Statement - SQL Injection 취약
...
ResultSet rs = pstmt.executeQuery();
// 입력값이 문자열로 처리되어 SQL Injection 방지`}</code></pre>
          </div>
          <div className="callout callout-tip">
            <div className="callout-title"><i className="fas fa-lightbulb"></i> 팁</div>
            <p>PreparedStatement는 보안뿐만 아니라 성능 면에서도 유리합니다. SQL이 미리 컴파일되어 캐시되므로, 같은 구조의 쿼리를 반복 실행할 때 더 빠릅니다.</p>
          </div>

          {/* JavaCodeRunner */}
          <h2>8. JDBC 개념 실습</h2>
          <p>JDBC의 핵심 개념인 Connection, PreparedStatement, ResultSet의 동작을 시뮬레이션합니다.</p>
...
}`} title="JDBC CRUD 및 SQL Injection 시뮬레이션" />

          {/* 연습문제 */}
          <div className="exercise-box">
            <h4><i className="fas fa-pencil-alt"></i> 연습문제</h4>
            <ol>
              <li>프로필 이미지를 업로드하고 DB에 파일 경로를 저장하는 서블릿을 구현하세요. 파일 크기는 5MB로 제한합니다.</li>
              <li>MySQL에 <code>products</code> 테이블을 만들고, JDBC를 사용하여 상품 CRUD를 구현하세요.</li>
              <li>Tomcat DBCP를 설정하고 <code>DBUtil</code> 클래스를 만들어 커넥션 풀을 사용하세요.</li>
              <li><code>Statement</code> 대신 <code>PreparedStatement</code>를 사용하여 기존 DAO를 리팩토링하세요.</li>
            </ol>
          </div>

          <div style={{marginTop:'32px', textAlign:'center'}}>
            <button className={`btn ${isLessonCompleted('S09') ? 'btn-secondary' : 'btn-accent'}`}
              onClick={() => completeLesson('S09')}>
              {isLessonCompleted('S09') ? '✓ 학습 완료!' : '학습 완료하기'}
            </button>
          </div>
          <div className="lesson-nav">
            <Link to="/servlet/08"><i className="fas fa-arrow-left"></i> 이전: MVC 패턴 구현</Link>
            <Link to="/servlet/10">다음: 미니 프로젝트: CRUD 게시판 <i className="fas fa-arrow-right"></i></Link>
          </div>
        </div>
      </div>
    </main>
