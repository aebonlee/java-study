...
export default function ServletLesson10() {
    ...
    <main className="java-lesson">
      <div className="page-header">
        <div className="container">
          ...
          <h1>Lesson 10. 미니 프로젝트: CRUD 게시판</h1>
          <p>지금까지 배운 서블릿, JSP, MVC, JDBC를 종합하여 완전한 CRUD 게시판을 구현합니다.</p>
        </div>
      </div>
      <div className="container">
...
└── pom.xml`}</code></pre>
          ...
          <h2>2. DB 설계</h2>
          <div className="code-block">
            <div className="code-header"><span className="file-name"><i className="fas fa-file-code"></i> schema.sql</span></div>
            <pre><code>{`-- 데이터베이스 생성
...
('JSP와 EL 활용', 'JSP에서 EL을 활용하는 방법입니다.', '최지원');`}</code></pre>
          </div>

          {/* 3. VO와 DAO 구현 */}
          <h2>3. VO와 DAO 구현</h2>
          <h3>BoardVO - 게시글 데이터 객체</h3>
          <div className="code-block">
            <div className="code-header"><span className="file-name"><i className="fas fa-file-code"></i> BoardVO.java</span></div>
            <pre><code>{`package com.example.board.model.vo;
...
public class BoardVO {
    ...
    public void setUpdatedAt(Timestamp updatedAt) { this.updatedAt = updatedAt; }
          ...
          <h3>BoardDAO - DB 접근 계층</h3>
          <div className="code-block">
            <div className="code-header"><span className="file-name"><i className="fas fa-file-code"></i> BoardDAO.java</span></div>
            <pre><code>{`package com.example.board.model.dao;
...
public class BoardDAO {
    ...
    private BoardVO mapRow(ResultSet rs) throws SQLException {
        ...
        return board;
          ...
          <h2>4. 서블릿 컨트롤러 구현</h2>
          <div className="code-block">
            <div className="code-header"><span className="file-name"><i className="fas fa-file-code"></i> BoardServlet.java</span></div>
...
public class BoardServlet extends HttpServlet {
    ...
    private void forward(HttpServletRequest req, HttpServletResponse resp,
        ...
        req.getRequestDispatcher(path).forward(req, resp);
          ...
          <div className="code-block">
            <div className="code-header"><span className="file-name"><i className="fas fa-file-code"></i> list.jsp</span></div>
...
<body>
    ...
    <a href="writeForm">글쓰기</a>
          ...
          <h3>상세 페이지 (view.jsp)</h3>
          <div className="code-block">
            <div className="code-header"><span className="file-name"><i className="fas fa-file-code"></i> view.jsp</span></div>
            <pre><code>{`<%@ page contentType="text/html;charset=UTF-8" %>
<%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt" %>
<html>
<body>
    ...
    <a href="list">목록</a>
          ...
          <h3>등록/수정 폼 (writeForm.jsp)</h3>
          <div className="code-block">
            <div className="code-header"><span className="file-name"><i className="fas fa-file-code"></i> writeForm.jsp</span></div>
            <pre><code>{`<%@ page contentType="text/html;charset=UTF-8" %>
<html>
<body>
    <h1>글쓰기</h1>
    <form action="create" method="post">
        ...
        <a href="list">취소</a>
          ...
          <h2>6. 페이징 처리</h2>
          <p>
            게시글이 많아지면 페이징이 필수입니다. 핵심은 MySQL의 <code>LIMIT</code>과 <code>OFFSET</code>입니다.
          </p>
          <div className="code-block">
            <div className="code-header"><span className="file-name"><i className="fas fa-file-code"></i> 페이징 로직 정리</span></div>
            <pre><code>{`// 페이징 핵심 공식
...
int endPage = Math.min(startPage + blockSize - 1, totalPages);`}</code></pre>
          </div>
          <div className="callout callout-tip">
            <div className="callout-title"><i className="fas fa-lightbulb"></i> 팁</div>
            <p>대용량 데이터에서는 OFFSET 기반 페이징 대신 커서(Cursor) 기반 페이징이 성능상 유리합니다. <code>WHERE id &lt; ? ORDER BY id DESC LIMIT 10</code> 방식을 고려하세요.</p>
          </div>

          {/* 7. 프로젝트 완성과 배포 */}
          <h2>7. 프로젝트 완성과 배포</h2>
          <div className="code-block">
            <div className="code-header"><span className="file-name"><i className="fas fa-file-code"></i> 배포 체크리스트</span></div>
            <pre><code>{`1. 프로젝트 빌드
   $ mvn clean package
...
4. 테스트
   ...
   - 페이징 동작 확인`}</code></pre>
          ...
          <div className="callout callout-info">
            <div className="callout-title"><i className="fas fa-info-circle"></i> 다음 단계</div>
            <p>이 게시판 프로젝트를 완성하면 Spring Framework로 동일한 기능을 구현해봅니다. Spring MVC가 서블릿 기반 MVC의 반복적인 코드를 얼마나 줄여주는지 체감할 수 있습니다.</p>
          </div>

          {/* JavaCodeRunner */}
          <h2>8. Board VO 클래스 실습</h2>
          <p>게시판 프로젝트의 핵심인 BoardVO 클래스를 만들고 활용해봅니다.</p>
...
}`} title="CRUD 게시판 시뮬레이션" />

          {/* 연습문제 */}
          <div className="exercise-box">
            <h4><i className="fas fa-pencil-alt"></i> 연습문제</h4>
            <ol>
              <li>이 게시판에 검색 기능을 추가하세요. 제목, 작성자, 내용으로 검색할 수 있어야 합니다.</li>
              <li>댓글(Comment) 기능을 추가하세요. 게시글 하나에 여러 댓글을 작성할 수 있어야 합니다.</li>
              <li>로그인/로그아웃 기능을 추가하고, 자신의 글만 수정/삭제할 수 있도록 제한하세요.</li>
              <li>파일 첨부 기능을 추가하여 게시글에 이미지나 문서를 업로드할 수 있게 구현하세요.</li>
              <li>이 프로젝트를 Spring MVC로 마이그레이션하여 코드량이 어떻게 줄어드는지 비교해보세요.</li>
            </ol>
          </div>

          <div style={{marginTop:'32px', textAlign:'center'}}>
            <button className={`btn ${isLessonCompleted('S10') ? 'btn-secondary' : 'btn-accent'}`}
              onClick={() => completeLesson('S10')}>
              {isLessonCompleted('S10') ? '✓ 학습 완료!' : '학습 완료하기'}
            </button>
          </div>
          <div className="lesson-nav">
            <Link to="/servlet/09"><i className="fas fa-arrow-left"></i> 이전: 파일 업로드와 DB 연동</Link>
            <Link to="/spring">다음: 스프링 과정 <i className="fas fa-arrow-right"></i></Link>
          </div>
        </div>
      </div>
    </main>
