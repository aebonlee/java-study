...
export default function SpringLesson15() {
    ...
    <main className="java-lesson">
      <div className="page-header">
        <div className="container">
          ...
          <h1>Lesson 15. Swagger/OpenAPI 문서화</h1>
          <p>OpenAPI 3.0 스펙과 SpringDoc 라이브러리를 활용하여 API 문서를 자동 생성하고 관리하는 방법을 배웁니다.</p>
        </div>
      </div>
      <div className="container">
        <div className="lesson-content">

          <h2>1. API 문서화의 중요성</h2>
          <p>API 문서는 프론트엔드 개발자, 외부 파트너, QA 팀이 API를 이해하고 사용하기 위한 <strong>필수 자산</strong>입니다.</p>

          <table className="info-table">
            <thead><tr><th>방법</th><th>장점</th><th>단점</th></tr></thead>
            <tbody>
              <tr><td><strong>수동 문서 (Wiki, Notion)</strong></td><td>자유로운 형식</td><td>코드와 동기화 어려움, 누락 위험</td></tr>
              <tr><td><strong>Postman Collection</strong></td><td>바로 테스트 가능</td><td>코드 변경 시 수동 업데이트</td></tr>
              <tr><td><strong>Swagger/OpenAPI (자동)</strong></td><td>코드 기반 자동 생성, 항상 최신</td><td>초기 설정 필요</td></tr>
            </tbody>
          </table>

          <div className="callout callout-info">
            <div className="callout-title"><i className="fas fa-info-circle"></i> Swagger vs OpenAPI</div>
            <p>OpenAPI는 REST API 명세의 표준 스펙이고, Swagger는 이 스펙을 구현하는 도구 모음입니다. Swagger UI는 OpenAPI 스펙을 시각적으로 보여주는 웹 인터페이스입니다. OpenAPI 3.0이 현재 최신 표준입니다.</p>
          </div>

          <h2>2. OpenAPI 3.0 스펙</h2>
          <div className="code-block">
            <div className="code-header"><span className="file-name"><i className="fas fa-file-code"></i> OpenAPI 3.0 스펙 예시 (YAML)</span></div>
...
// OpenAPI YAML: http://localhost:8080/v3/api-docs.yaml`}</code></pre>
          </div>

          <div className="code-block">
            <div className="code-header"><span className="file-name"><i className="fas fa-file-code"></i> application.yml (설정)</span></div>
            <pre><code>{`springdoc:
  swagger-ui:
  ...
  default-produces-media-type: application/json`}</code></pre>
          </div>

          <div className="code-block">
            <div className="code-header"><span className="file-name"><i className="fas fa-file-code"></i> OpenApiConfig.java</span></div>
...
public class OpenApiConfig {
    ...
    public OpenAPI openAPI() {
        return new OpenAPI()
          ...
          <h2>4. @Operation, @ApiResponse, @Parameter 어노테이션</h2>
          <div className="code-block">
            <div className="code-header"><span className="file-name"><i className="fas fa-file-code"></i> PostController.java (문서화 적용)</span></div>
...
public class PostController {
    ...
    public ResponseEntity<PostDto> createPost(
        ...
        return ResponseEntity.status(HttpStatus.CREATED).body(created);
          ...
          <h2>5. @Schema로 모델 문서화</h2>
          <div className="code-block">
            <div className="code-header"><span className="file-name"><i className="fas fa-file-code"></i> DTO에 @Schema 적용</span></div>
            <pre><code>{`@Schema(description = "게시글 생성 요청")
...
public enum PostStatus {
    ...
    @Schema(description = "삭제됨") DELETED
          ...
          <h2>6. Swagger UI 커스터마이징</h2>
          <div className="code-block">
            <div className="code-header"><span className="file-name"><i className="fas fa-file-code"></i> 커스터마이징 옵션</span></div>
            <pre><code>{`// application.yml에서 Swagger UI 커스터마이징
springdoc:
  swagger-ui:
...
springdoc:
  ...
  swagger-ui:
    enabled: false`}</code></pre>
          ...
          <h2>7. 그룹별 API 문서 분리</h2>
          <div className="code-block">
            <div className="code-header"><span className="file-name"><i className="fas fa-file-code"></i> 그룹 설정</span></div>
            <pre><code>{`@Configuration
...
// /swagger-ui.html?urls.primaryName=user-api`}</code></pre>
          </div>

          <div className="callout callout-tip">
            <div className="callout-title"><i className="fas fa-lightbulb"></i> 팁</div>
            <p>Swagger 문서는 개발/테스트 환경에서만 활성화하고, 프로덕션에서는 반드시 비활성화하세요. 또한 민감한 API (관리자 전용 등)는 별도 그룹으로 분리하여 접근을 제한하는 것이 좋습니다.</p>
          </div>

          <JavaCodeRunner defaultCode={`// OpenAPI 어노테이션 개념 이해하기
...
}`} title="OpenAPI 어노테이션 개념 데모" />

          <div className="exercise-box">
            <h4><i className="fas fa-pencil-alt"></i> 연습문제</h4>
            <ol>
              <li>SpringDoc을 프로젝트에 추가하고 Swagger UI에 접속해 보세요.</li>
              <li>모든 컨트롤러에 @Operation, @ApiResponse, @Parameter를 적용해 보세요.</li>
              <li>DTO 클래스에 @Schema를 적용하여 필드 설명과 예시를 추가해 보세요.</li>
              <li>API를 2개 이상의 그룹으로 분리하고, 프로덕션 환경에서 Swagger를 비활성화하는 설정을 해 보세요.</li>
            </ol>
          </div>

          <div style={{marginTop:'32px', textAlign:'center'}}>
            <button className={`btn ${isLessonCompleted('SP15') ? 'btn-secondary' : 'btn-accent'}`}
              onClick={() => completeLesson('SP15')}>
              {isLessonCompleted('SP15') ? '✓ 학습 완료!' : '학습 완료하기'}
            </button>
          </div>
          <div className="lesson-nav">
            <Link to="/spring/14"><i className="fas fa-arrow-left"></i> 이전: 테스트 (JUnit5, MockMvc)</Link>
            <Link to="/spring/16">다음: 배포 (Docker, CI/CD) <i className="fas fa-arrow-right"></i></Link>
          </div>
        </div>
      </div>
    </main>
